...
async function aiMarketInsight(price, change) {
  try {
    const prompt = `
BTC price is currently $${price}.
Market movement changed by ${change.toFixed(2)}%.

Give a short crypto analyst explanation of the market movement.
...
async function analyzeAsset(name, url, ctx) {
  try {
    ...
    const insight = await aiMarketInsight(price, change);
...
function explainBTC(change) {
  if (change > 2) {
    return "📈 Strong bullish momentum. Buyers are in control.";
  }

  if (change < -2) {
    return "📉 Bearish pressure. Sellers are dominating.";
  }

  return "⚖️ Market is neutral. No strong movement.";
}


    const price = res.data.bitcoin.usd;

    const trend = price > 65000 ? "Bullish 📈" : "Bearish 📉";
...
Insight:
${price > 65000 
  ? "Buyers are currently stronger in the market."
  : "Sellers are currently dominating short-term movement."}`
    );
  } catch (err) {
    console.error(err);
    ctx.reply("⚠️ Failed to fetch BTC data");
  }
});
...
Price: $${price}
Trend: ${price > 65000 ? "Bullish 📈 (above reference zone)" : "Bearish 📉 (below reference zone)"}

Insight:
${price > 65000 
  ? "Momentum suggests buyers are active in the market."
  : "Selling pressure may be increasing in short term."}
`;

    ctx.reply(reply);
  } catch (err) {
    console.error(err);
    ctx.reply("⚠️ Failed to fetch BTC price");
  }
});
...
function analyzeChange(current, previous) {
  ...
  let insight = "Market is stable with no strong direction.";

  if (change > 2) {
    sentiment = "bullish 📈";
    insight = "Strong buying pressure detected. Momentum favors buyers.";
  } else if (change < -2) {
    sentiment = "bearish 📉";
    insight = "Selling pressure increasing. Bears are in control.";
  }

  return { change, sentiment, insight };
}

  try {
    const price = await getBTCData();

    const previous = lastPrice || price;
    const analysis = analyzeChange(price, previous);
...
Price: $${price}
Change: ${analysis.change.toFixed(2)}%
...
${analysis.insight}`
    ...
    ctx.reply("⚠️ Unable to fetch market data");
  }
});
...
${insight}`
    ...
    console.error(err);
    ctx.reply("⚠️ Unable to fetch price");
  }
});
